                        ...
                        const knot_xdp_msg_t *in_reply_to)
{
	...
	if (in_reply_to == NULL) {
		...
		knot_xdp_msg_answer(out, uframe, FRAME_SIZE, in_reply_to);
	}

	return KNOT_EOK;
}

static uint8_t *uframe_find_header(uint8_t *uframe_p)
{
	if (knot_xdp_zero_header(uframe_p)) {
		uframe_p += XDP_PACKET_HEADROOM;
	}
	return uframe_p;
}
...
#ifndef NDEBUG
	uint8_t *hdr_p = uframe_find_header(uframe_p);

	knot_xdp_payload_t p = {
		.buf = uframe_p,
		.len = msg->payload.iov_len + (msg->payload.iov_base - hdr_p),
		.err = KNOT_EOK,
                  ...
                  uint32_t count, uint32_t *sent)
{
	...
	for (uint32_t i = 0; i < count; ++i) {
		...
		if (!knot_xdp_empty_msg(msg) && ret == KNOT_EOK) {
			uint8_t *uframe_p = uframe_find_header(msg_uframe_ptr(socket, msg));
			size_t uframe_len = msg->payload.iov_len + (msg->payload.iov_base - uframe_p);
