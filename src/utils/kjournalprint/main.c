...
int print_journal(char *path, knot_dname_t *name, print_params_t *params)
{
	...
	if (params->limit >= 0 && ret == KNOT_EOK) {
		if (params->from_serial < 0) {
			...
			ret = journal_walk_from(j, params->from_serial, count_changeset_cb, params);
		}
	}
	if (ret == KNOT_EOK) {
		...
		if (params->from_serial < 0) {
			...
			ret = journal_walk_from(j, params->from_serial, print_changeset_cb, params);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "l:s:nzcdhV", opts, NULL)) != -1) {
		...
		case 'l':
			...
			break;
		case 's':
			if (str_to_long(optarg, &params.from_serial, 0, UINT32_MAX) != KNOT_EOK) {
				print_help();
				return EXIT_FAILURE;
			}
			break;
